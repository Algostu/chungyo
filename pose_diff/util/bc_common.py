...
class Common():
    ...
    def apply_vector(self, ex_type, length, vector):
        ...
        for i in range(frames_len):
            ...
            if parts[11] > 0:
                # n += 1
                frames[i][11] = self.calc_coordinates(frames[i][1], tuple(vector[i][8]), basic_length[7])

            # # nose
            # if parts[0] > 0:
            #     # n += 1
            #     frames[i][0] = self.calc_coordinates(frames[i][1], tuple(vector[i][9]), basic_length[8])
